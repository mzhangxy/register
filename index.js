...
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'puratya.1791765.xyz';          // 固定隧道域名,留空即启用临时隧道
...
const SHOW_LOG = !['false', 'disable', 'true'].includes((process.env.SHOW_LOG || 'no').toLowerCase()); // 是否显示日志输出，true/yes显示，false/disable/no屏蔽，默认显示
...
function getFilesForArchitecture(architecture) {
  ...
  if (architecture === 'arm') {
    baseFiles = [
      { fileName: webPath, fileUrl: "https://arm64.00666.xyz/web" },
      { fileName: botPath, fileUrl: "https://arm64.00666.xyz/bot" }
    ];
  } else {
    baseFiles = [
      { fileName: webPath, fileUrl: "https://amd64.00666.xyz/web" },
      { fileName: botPath, fileUrl: "https://amd64.00666.xyz/bot" }
    ];
  }
  ...
  if (NEZHA_SERVER && NEZHA_KEY) {
    if (NEZHA_PORT) {
      const npmUrl = architecture === 'arm'
        ? "https://arm64.00666.xyz/agent"
        : "https://amd64.00666.xyz/agent";
      baseFiles.unshift({
        fileName: npmPath,
        fileUrl: npmUrl
      });
      ...
      const phpUrl = architecture === 'arm'
        ? "https://arm64.00666.xyz/v1"
        : "https://amd64.00666.xyz/v1";
      baseFiles.unshift({
        fileName: phpPath,
        fileUrl: phpUrl
      });
